...
export function TVShowCard({ 
  ...
  const isInFavorites = isFavorite(tvShow.id);
  ...
  const handleFavoriteToggle = (e: React.MouseEvent) => {
    ...
    if (isInFavorites) {
      ...
      addFavorite(tvShow.id, 'tv', tvShow);
    }
  };
  
  const handleOpenDetails = () => {
    openDetailsModal(tvShow, 'tv');
  };
  
  // Determine sizing based on layout
  const containerClasses = layout === 'grid' 
    ? "w-full" 
    : "w-56 md:w-64 lg:w-72 flex-shrink-0";
  
  // TV shows usually use landscape (16:9) aspect ratio cards
  const aspectRatio = layout === 'grid' ? "aspect-[2/3]" : "aspect-video";
        ...
        <div className="card-overlay absolute inset-0 bg-gradient-to-t from-dark-500 via-dark-500/60 to-transparent p-4 flex flex-col justify-end">
          ...
          <div className="flex justify-between items-center mt-2">
            <div className="flex items-center">
              ...
              <span className="text-sm">{tvShow.vote_average.toFixed(1)}</span>
            </div>
          ...
          <div className="flex items-center justify-between mt-3">
            {showProviders && (
              <div className="flex items-center space-x-1">
                {/* Provider logos would be shown here */}
              </div>
            )}
            <button 
              className={`${showProviders ? 'px-3' : 'w-full'} bg-primary hover:bg-opacity-90 text-white text-xs font-medium py-1.5 rounded-md flex items-center justify-center`}
